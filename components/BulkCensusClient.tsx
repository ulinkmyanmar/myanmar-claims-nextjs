    ...
    <div className="mt-6">
      ...
      <div className="mt-8 rounded-3xl border border-line bg-white p-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <h2 className="text-xl font-bold">Sample bulk result</h2>
            <p className="mt-2 text-muted">
              Click any member row to view census details and available historical claims.
            </p>
          </div>

          <button type="button" className="btn-secondary" onClick={loadSampleResult}>
            Load sample result
          </button>
        </div>
                ...
                {SAMPLE_ROWS.map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td>
                        <span
                          className={row.status === "Matched" ? "pill pill-green" : "pill pill-gray"}
                        >
                          {row.status}
                        </span>
                      </td>
                      <td>
                        {typeof firstClaim?.id === "string" && firstClaim.id.startsWith("HM")
                          ? firstClaim.id
                          : "—"}
                      </td>
                    </tr>
      ...
      {fileName && (
        <div className="mt-6 rounded-xl border border-line bg-blue-50 p-4">
          ...
          <p className="mt-1 text-sm text-muted">{rows.length} member(s) loaded</p>

          {searching && (
            <p className="mt-2 text-sm font-semibold text-brand">Checking historical claims...</p>
          )}
        </div>
      )}
            ...
            <table className="w-full">
              ...
              <tbody>
                {rows.map((row, index) => {
                  const firstClaim = row.claims?.[0]
                  const historicalId =
                    firstClaim && typeof firstClaim.historical_member_id === "string"
                      ? firstClaim.historical_member_id
                      : "—"
      ...
      {selectedRow && (
        <div className="mt-6 rounded-3xl border border-line bg-white p-6">
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            <div>
              <p className="text-sm text-muted">Uploaded Member</p>
              <p className="font-semibold">{selectedRow.name || "—"}</p>
            </div>

            <div>
              <p className="text-sm text-muted">NRC / National ID</p>
              <p className="font-semibold">{selectedRow.nrc || "—"}</p>
            </div>

            <div>
              <p className="text-sm text-muted">Date of Birth</p>
              <p className="font-semibold">{selectedRow.dob || "—"}</p>
            </div>

            <div>
              <p className="text-sm text-muted">Gender</p>
              <p className="font-semibold">{selectedRow.gender || "—"}</p>
            </div>

            <div>
              <p className="text-sm text-muted">Match Status</p>
              <span className={selectedRow.status === "Matched" ? "pill pill-green" : "pill pill-gray"}>
                {selectedRow.status}
              </span>
            </div>
          </div>
          ...
          {/* Matching claims */}
          <div className="mt-8">
            <h3 className="text-lg font-bold">Historical Claims</h3>
            ...
            {selectedRow.claims?.length === 0 ? (
              ...
              <div className="mt-3 grid gap-4">
                {selectedRow.claims.map((claim, index) => (
                  <div key={index} className="rounded-xl border border-line bg-stone-50 p-4">
                    <div className="mb-3 flex items-center gap-2">
                      <Search size={18} />
                      <b>Claim Record {index + 1}</b>
                    </div>
